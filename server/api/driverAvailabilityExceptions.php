...
switch ($_SERVER['REQUEST_METHOD']) {
   case "GET":
      if (isset($_GET['id']))
      {
         echo json_encode($db->driverAvailabilityExceptionDAO->getDriverAvailabilityException($_GET['id']));
      }
      else if (isset($_GET['page']) && isset($_GET['number_per_page']))
      {
         echo json_encode($db->driverAvailabilityExceptionDAO->getDriverAvailabilityExceptions($_GET['page'], $_GET['number_per_page']?:10));
      }
      else
      {
         echo json_encode($db->driverAvailabilityExceptionDAO->getDriverAvailabilityExceptions(0, 100));
      }
      break;
   ...
   case "POST":
      ...
      if (isset($bodyData['start']) && isset($bodyData['end']) && isset($bodyData['driverID']))
      {
         ...
         echo json_encode($db->driverAvailabilityExceptionDAO->insertDriverAvailabilityException($startTime, $endTime, $driverID));
      }
      else
      {
   ...
   case "PUT":
      ...
      if (isset($bodyData['id']))
      {
         ...
         http_response_code(201);
         echo json_encode($db->driverAvailabilityExceptionDAO->updateDriverAvailabilityException($id, $startTime, $endTime, $driverID));
      }
      else
      {
         http_response_code(404);
   ...
   case "DELETE":
      ...
      if (isset($bodyData['id']))
      {
         ...
         http_response_code(201);
         echo json_encode($db->driverAvailabilityExceptionDAO->deleteDriverAvailabilityException($id));
      }
      else
      {
         http_response_code(404);
